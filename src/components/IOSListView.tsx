...
export default function SwipableList() {
  const [items, setItems] = useState(messages.emails);
  ...
  return (
    <div className="w-full h-full flex justify-center items-center p-4">
      <div className="flex flex-col w-full h-full max-h-[500px] sm:w-[400px] -mx-[14px] sm:mx-0">
        {/* Toolbar */}
...
function ListItem({
  ...
  useMotionValueEvent(x, "change", (x) => {
    const a = x < -ref.current!.offsetWidth * 0.8 ? "start" : "end";
    setAlign(a);
  });
...
function SelectionCheckmark({ isSelected }: { isSelected: boolean }) {
  return (
    <motion.svg
      aria-hidden="true"
      viewBox="0 0 24 24"
