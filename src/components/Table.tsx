...
export default function StockTableExample() {
  ...
  const sortedItems = useMemo(() => {
    return stocks.sort((a, b) => {
      const first = a[
        sortDescriptor.column as keyof (typeof stocks)[number]
      ] as string;
      const second = b[
        sortDescriptor.column as keyof (typeof stocks)[number]
      ] as string;
  ...
  return (
    <div className="bg-gradient-to-r from-indigo-500 to-violet-500 p-2 lg:p-8 w-full h-full flex items-center justify-center md:col-span-2">
      <ResizableTableContainer className="text-2xs md:text-base max-h-[280px] w-full overflow-x-hidden overflow-y-auto relative bg-white rounded-lg shadow text-gray-600">
        <Table
          aria-label="Stocks"
